...
- (void) createIconItemCell
{
    ...
    UIImage *image = [LXBundleTool getImage:self.items.icon FromBundle:LXFrameWorkBundle];
    if (image) {
        _iconImg.image = image;
    }else{
        [_iconImg sd_setImageWithURL:[NSURL URLWithString:self.items.icon] placeholderImage:[LXBundleTool getImage:@"user_defaultavatar" FromBundle:LXFrameWorkBundle]];
    }
...
{
    if (!_imgArrowView) {
        _imgArrowView = [[UIImageView alloc] initWithImage:[LXBundleTool getImage:@"Arrow_Left" FromBundle:LXFrameWorkBundle]];
    }
